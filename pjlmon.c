...
//#define WAIT_FOR_WRITE                  600 // 0.5 sec
//#define WAIT_FOR_DATA_TIMEOUT           500 // 0.5 sec
//#define WAIT_FOR_USTATUS_THREAD_TIMEOUT 500 // 0.5 sec
#define GETDEVICEID                     IOCTL_PAR_QUERY_DEVICE_ID
#define MAX_DEVID                       1024

#define HPLJ1000 "MFG:Hewlett-Packard;MDL:hp LaserJet 1000;CMD:ZJS;CLS:PRINTER;DES:hp LaserJet 1000;"
#define HPLJ1000_FW "MFG:Hewlett-Packard;MDL:hp LaserJet 1000;CMD:ZJS;CLS:PRINTER;DES:hp LaserJet 1000;FWVER:20011207;"
...
--*/
{
	...
	*pcbWritten=0;
    ...
    if ( pIniPort->pIniJob &&
        ...
        WriteCommand(hPort, "@PJL USTATUS JOB = ON \015\012@PJL USTATUS PAGE = ON \015\012@PJL USTATUS DEVICE = ON \015\012");
		WriteCommand(hPort, "@PJL INFO MEMORY\015\012@PJL INFO STATUS\015\012");
		WriteCommand(hPort, "@PJL JOB\015\012");
		WriteCommand(hPort, "@PJL SET DENSITY=31\015\012@PJL SET JAMRECOVERY=OFF\015\012@PJL SET TRANSFERMODE=NORMAL\015\012@PJL ENTER LANGUAGE=ZJS\015\012");
    }
    ...
    //
	pbuf=pBuffer;
	for (i=0;i<cbBuf/4096;i++)
	{

		ret = (*pIniPort->fn.pfnWritePort)(pIniPort->hPort, pbuf,
                                       4096, &written);
		*pcbWritten+=written;
		pbuf+=4096;
...
--*/
{
    ...
    if ( pIniJob && (pIniJob->status & PP_PJL_SENT) ) {
        ...
        WriteCommand(hPort, string);*/
		WriteCommand(hPort, "\033%-12345X@PJL EOJ\015\012@PJL ENTER LANGUAGE=ZJS\015\012");
...
--*/
{
    ...
    if (pIniPort->fn.pfnGetPrinterDataFromPort) {
        ...
        if ((*pIniPort->fn.pfnGetPrinterDataFromPort)
			...
			//check printer TYPE
			if( strlen(szID)==strlen(HPLJ1000))
			{
				if(mystrncmp(szID,HPLJ1000,strlen(HPLJ1000))==0)
				{
					DWORD len=SIZE_FW1000;
					DWORD i,pcbWritten;
					PBYTE pBuffer=fw1000;
					for(i=0;i<len/4096;i++)
					{
						(*pIniPort->fn.pfnWritePort)(pIniPort->hPort, pBuffer,
							4096, &pcbWritten);
						pBuffer+=4096;
					}
					if(len%4096)
            ...
            if (lpCMD) {
                ...
                while (*lpCMD) {
					...
					if ( lpCMD[0] == 'Z' && lpCMD[1] == 'J' && lpCMD[2] == 'S' ){
                        pIniPort->status &= ~PP_DONT_TRY_PJL;
                        bRet = TRUE;
                        goto Cleanup;
                    }
